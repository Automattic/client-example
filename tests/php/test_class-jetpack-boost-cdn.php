<?php
include_once dirname( __FILE__ ) . '/../../includes/class-jetpack-boost-cdn.php';

class WP_Test_Jetpack_Boost_CDN extends WP_UnitTestCase {
	var $old_wp_styles;
	var $old_wp_scripts;
	...
	public function setUp() {
		...
		add_filter( 'jetpack_boost_cdn_url', array( $this, 'cdn_url' ) );

		Jetpack_Boost_CDN::reset();
		Jetpack_Boost_CDN::instance();
	}

	public function tearDown() {

		remove_filter( 'jetpack_boost_cdn_url', array( $this, 'cdn_url' ) );
	...
	public function test_concatenates_css() {
		wp_enqueue_style( 'my-style', plugins_url( 'css/my-style.css' ), false, '1.0' );
		wp_enqueue_style( 'other-style', plugins_url( 'css/other-style.css' ), false, '2.0' );
		...
		$this->assertArraySubset( array(
			$this->strip_host( plugins_url( 'css/my-style.css' ) ),
			$this->strip_host( plugins_url( 'css/other-style.css' ) )
		), $query['f'] );
		...
		wp_enqueue_style( 'footer-style', plugins_url( 'css/footer-style.css' ), false, '3.0' );
		wp_enqueue_style( 'footer-style-too', plugins_url( 'css/footer-style-too.css' ), false, '4.0' );
		$cdn_css_urls = $this->get_cdn_css_urls( $this->get_footer_content() );
		$query = $cdn_css_urls[0]->query;

		$this->assertArraySubset( array(
			$this->strip_host( plugins_url( 'css/footer-style.css' ) ),
			$this->strip_host( plugins_url( 'css/footer-style-too.css' ) )
		), $query['f'] );
	...
	public function test_separates_css_by_media() {
		wp_enqueue_style( 'my-style', plugins_url( 'css/my-style.css' ), false, '1.0', 'all' );
		wp_enqueue_style( 'other-style', plugins_url( 'css/other-style.css' ), false, '2.0', 'print' );
		wp_enqueue_style( 'yet-other-style', plugins_url( 'css/yet-other-style.css' ), false, '3.0', 'all' );
		...
		$this->assertArraySubset( array(
			$this->strip_host( plugins_url( 'css/my-style.css' ) ),
			$this->strip_host( plugins_url( 'css/yet-other-style.css' ) )
	...
	public function test_doesnt_concat_conditional_css() {
		wp_enqueue_style( 'my-style', plugins_url( 'css/my-style.css' ), false, '1.0', 'all' );
		wp_enqueue_style( 'conditional-style', plugins_url( 'css/other-style.css' ), false, '2.0', 'print' );
	...
	public function test_concatenates_js() {
		...
		wp_enqueue_script( 'my-script', plugins_url( 'js/my-script.js' ), false, '1.0' );
		wp_enqueue_script( 'other-script', plugins_url( 'js/other-script.js' ), false, '2.0' );
		wp_enqueue_script( 'footer-script', plugins_url( 'js/footer-script.js' ), false, '3.0', true );

		$content = $this->get_head_content();

		// assert old script tags are not there
		$existingScriptPath = plugins_url( 'js/my-script.js' ) . '?ver=1.0';
		...
		// should include URLs without hostname
		$this->assertArraySubset( array(
			$this->strip_host( plugins_url( 'js/my-script.js' ) ),
			$this->strip_host( plugins_url( 'js/other-script.js' ) )
		), $query['f'] );
		...
		// should include URLs without hostname
		$this->assertArraySubset( array(
			$this->strip_host( plugins_url( 'js/footer-script.js' ) )
		), $query['f'] );
	...
	public function test_breaks_js_on_intervening_non_CDN_script() {
		wp_enqueue_script( 'my-script', plugins_url( 'js/my-script.js' ), false, '1.0' );
		wp_enqueue_script( 'non-cdn-script', plugins_url( 'js/non-cdn-script.js' ), false, '2.0' );
		wp_enqueue_script( 'next-cdn-script', plugins_url( 'js/next-cdn-script.js' ), false, '3.0' );
		wp_enqueue_script( 'another-cdn-script', plugins_url( 'js/another-cdn-script.js' ), false, '4.0' );

		add_filter( 'jetpack_boost_concat_script', array( $this, 'dont_concat_non_cdn_script' ), 10, 3 );
		...
		$this->assertArraySubset( array(
			$this->strip_host( plugins_url( 'js/next-cdn-script.js' ) ),
			$this->strip_host( plugins_url( 'js/another-cdn-script.js' ) )
	...
	public function test_doesnt_concat_conditional_js() {
		wp_enqueue_script( 'my-script', plugins_url( 'js/my-script.js' ), false, '1.0' );
		wp_enqueue_script( 'conditional-script', plugins_url( 'js/other-script.js' ), false, '2.0' );
	...
	private function get_cdn_css_urls( $content ) {
		...
		$cdn_url = Jetpack_Boost_CDN::instance()->cdn_server;
